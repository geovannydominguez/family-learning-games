...
Every question text must be unique across the 10 questions. Within one question, no two answer texts may be identical. Prefer every answer text to be unique across the whole game when practical. Do not reuse the same sentence with only minor wording changes.
Before returning the JSON, internally verify that there are 10 unique question texts, that every question has 4 distinct answer texts, and that every question has exactly one correct answer; if any check fails, revise the content until it passes.
